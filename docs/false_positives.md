...
Fires on `transfer` / `transfer_from` method calls on a receiver whose type is an ADT that does not match any known `soroban_sdk` type, when the call sits inside a loop.

- **Known false positive — loop bounded by a small contract-controlled constant:** if the loop iterates over a small, fixed, contract-controlled collection (e.g. `for fee in [bps_1, bps_2]` where the slice length is a compile-time constant), the per-iteration cost is bounded and intentional. Suppress with `#[allow(token_transfer_in_loop)]` for such cases.
- **Near-miss 1 — `transfer` on a non-token struct:** a plain Rust struct with a method named `transfer` (e.g. an internal `LedgerTransfer` helper) will fire if called inside a loop. The lint matches on the method name alone when the receiver is not a known SDK type. Suppress if the method does not represent a cross-contract token transfer.
- **Near-miss 2 — `Env::invoke_contract` is not matched:** calling `env.invoke_contract(&token, &symbol_short!("transfer"), args)` inside a loop fires `contract_call_in_loop` but not `token_transfer_in_loop`, because the receiver is `Env` (a known SDK type). The two lints are complementary; a user seeing both should understand that the generic lint catches the direct `invoke_contract` path while this lint catches the generated-client path.
- **Analysis is method-name-based:** the lint does not resolve the callee's trait or type hierarchy. Any method named `transfer` or `transfer_from` on a non-SDK-type ADT receiver inside a loop will fire, regardless of whether it actually represents a token transfer. This is intentional — the false-positive surface is small and the fix advice (claim pattern) is broadly applicable to any cross-contract push-in-a-loop.
